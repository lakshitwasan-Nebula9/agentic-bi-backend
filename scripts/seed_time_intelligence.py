"""
Seed script — rebuilds the `orders` table in the local source DB with 14 months
of data (April 2025 – May 2026) so there are enough monthly snapshots to test:
  - YoY  (needs ≥13 months)
  - QoQ  (needs ≥4 months)
  - YTD  (needs ≥1 snapshot in current year)

After running this script:
  1. In the app, re-sync the orders dataset  →  POST /api/v1/datasets/{id}/sync
  2. Trigger KPI recompute                   →  POST /api/v1/kpis/{id}/recompute  (or via KPI agent)
  3. Hit GET /api/v1/kpis to see yoy_change_pct, qoq_change_pct, ytd_value

Usage:
    python scripts/seed_time_intelligence.py

Requires local Postgres on localhost:5432 (same credentials as seed_demo_db.py).
"""
...
def month_range(start: datetime, end: datetime):
    """Yield (month_start, month_end) pairs between start and end inclusive."""
...
def build_orders(month_start: datetime, month_end: datetime, id_offset: int) -> list[dict]:
    """Generate ORDERS_PER_MONTH orders for a single calendar month."""
    orders = []
    ...
    if month in (11, 12):
        ...
        volume_mult = random.uniform(1.05, 1.2)  # summer bump
    else:
...
def main() -> None:
    ...
    with conn.cursor() as cur:
        # Drop and recreate so schema is always clean
        ...
        for month_start, month_end in months:
            rows = build_orders(month_start, month_end, id_offset)
            cur.executemany(
                """
                INSERT INTO orders
            ...
            print(f"  {label}: {len(rows):>4} orders  |  net revenue ≈ ${net:>10,.0f}")
