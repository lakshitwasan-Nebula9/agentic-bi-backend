...
def detect_insights(db: Session = Depends(get_db)):
    ...
    """
    return insight_service.detect_all(db)
...
def detect_for_kpi(kpi_id: uuid.UUID, db: Session = Depends(get_db)):
    """Run detection for a single KPI. Returns null when conditions aren't met."""
    return insight_service.detect_for_kpi(db, kpi_id)
